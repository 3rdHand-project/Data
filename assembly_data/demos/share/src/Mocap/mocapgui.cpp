...
struct sMocapGui: OpenGL::GLKeyCall {
...
void sMocapGui::set_target(Target target) {
    CHECK(rec.checkTarget(target), STRING("Sorry, target " << Target_to_str(target) << " not available for this demonstration."))
  ...
  if(target == TS) {
    ...
    for(const String &agent: agents) {
     ...
     // interactions.(*rec.kvgann.getItem(agent, objsensor)->getValue<KeyValueGraph>()->getValue<arr>("ann"));
      }
    }
