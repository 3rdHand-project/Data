...
struct MocapID::sMocapID {
  ...
  KeyValueGraph kvg, kvg_hid, kvg_sid;
  StringA sensors,
  ...
  void parseAgentSensor(const String &sensor, KeyValueGraph &kvg);
  void parseObjectSensor(const String &sensor, KeyValueGraph &kvg);
...
void MocapID::sMocapID::parseAgentSensor(const String &sensor, KeyValueGraph &kvg) {
...
void MocapID::sMocapID::parseObjectSensor(const String &sensor, KeyValueGraph &kvg) {
  CHECK_EQ(sensor(0), '/', "Sensor name must start with '/'.");
...
void MocapID::load(const char *meta) {
  ...
  KeyValueGraph *kvg_agents = kvg.getValue<KeyValueGraph>("agents");
  if(kvg_agents == nullptr)
    cout << "No Agents!" << endl;
  else
    for(Item* i: *kvg_agents)
      s->parseAgentSensor(i->keys(0), *i->getValue<KeyValueGraph>());

  KeyValueGraph *kvg_objects = kvg.getValue<KeyValueGraph>("objects");
  if(kvg_objects == nullptr)
    cout << "No Objects!" << endl;
  else
    for(Item *i: *kvg_objects)
      s->parseObjectSensor(i->keys(0), *i->getValue<KeyValueGraph>());
}
...
void MocapRec::save() {
  ...
  for(Item *pair: kvgann) {
    for(Item *lock: *pair->getValue<KeyValueGraph>()) {
      if(lock->keys(0) == "lock") {
        value.clear();
        value["on@"] = (uint)*lock->getValue<KeyValueGraph>()->getValue<double>("from");
        value["off@"] = (uint)*lock->getValue<KeyValueGraph>()->getValue<double>("to");
...
bool MocapRec::checkTarget(Target target) {
  if(target == NO_TARGET)
    return true;
  return mlabel.getItem(Target_to_str(target)) != NULL;
}
